...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            }).on('kickedBannedWalletAddress', function (remainingBanTime) {
                emitLog('Rejected incoming connection from wallet address ' + walletAddress + ' banned for ' + remainingBanTime + ' more seconds');

            }).on('forgaveBannedWalletAddress', function () {
                emitLog('Forgave banned wallet address ' + walletAddress);
            
            }).on('kickedBannedWorkerName', function (remainingBanTime) {
                emitLog('Rejected incoming connection from worker name ' + client.workerName + ' banned for ' + remainingBanTime + ' more seconds');

            }).on('forgaveBannedWorkerName', function () {
                emitLog('Forgave banned worker name ' + client.workerName);
