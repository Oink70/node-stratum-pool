...
var StratumServer = exports.Server = function StratumServer(options, authorizeFn){
    ...
    const defaultBanTimeInSeconds = 9999999;
    var bannedMS = options.banning ? options.banning.time * 1000 : null;
    var bannedWalletAddressMS = options.banningWalletAddress ? options.banningWalletAddress.time * 1000 : null;
    var bannedWorkerNameMS = options.banningWorkerName ? options.banningWorkerName.time * 1000 : null;
    ...
    var bannedIPs = {};
    var bannedWalletAddresses = {};
    var bannedWorkerNames = {};


    function checkBan(client) {
        checkBanningIpAddress(client);
        checkBanningWalletAddress(client);
        checkBanningWorkerName(client);
    }

    function checkBanningIpAddress(client) {
        if (options.banning && options.banning.enabled) {
            if (options.banning.banned 
                && options.banning.banned.includes(client.remoteAddress)
                && bannedIPs[client.remoteAddress] === undefined) {
                client.socket.destroy();
                client.emit('kickedBannedIP', defaultBanTimeInSeconds);
                return
            }
    ...
    function checkBanningWalletAddress(client) {
        if (options.banningWalletAddress && options.banningWalletAddress.enabled) {
            const walletAddress = client.workerName.split(".")[0];
            if (options.banningWalletAddress.banned 
                && options.banningWalletAddress.banned.includes(walletAddress) 
                && bannedWalletAddresses[walletAddress] === undefined) {
                this.addBannedWalletAddress(walletAddress);
                client.socket.destroy();
                client.emit('kickedBannedWalletAddress', defaultBanTimeInSeconds);
                return
            }

            if (walletAddress in bannedWalletAddresses) {
                var bannedTime = bannedWalletAddresses[walletAddress];
                var bannedTimeAgo = Date.now() - bannedTime;
                var timeLeft = bannedWalletAddressMS - bannedTimeAgo;
                if (timeLeft > 0) {
                    client.socket.destroy();
                    client.emit('kickedBannedWalletAddress', timeLeft / 1000 | 0);
                } else {
                    delete bannedWalletAddresses[walletAddress];
                    client.emit('forgaveBannedWalletAddress');
                }
            }
        }
    }
    
    function checkBanningWorkerName(client) {
        if (options.banningWorkerName && options.banningWorkerName.enabled) {
            const workerName = client.workerName;
            if (options.banningWorkerName.banned 
                && options.banningWorkerName.banned.includes(workerName) 
                && bannedWorkerNames[workerName] === undefined) {
                this.addBannedWorkerName(workerName);
                client.socket.destroy();
                client.emit('kickedBannedWorkerName', defaultBanTimeInSeconds);
                return
            }

            if (workerName in bannedWorkerNames) {
                var bannedTime = bannedWorkerNames[workerName];
                var bannedTimeAgo = Date.now() - bannedTime;
                var timeLeft = bannedWorkerNameMS - bannedTimeAgo;
                if (timeLeft > 0) {
                    client.socket.destroy();
                    client.emit('kickedBannedWorkerName', timeLeft / 1000 | 0);
                } else {
                    delete bannedWorkerNames[workerName];
                    client.emit('forgaveBannedWorkerName');
                }
            }
        }
    }
        ...
        var client = new StratumClient(
            {
                ...
                banningWalletAddress: options.banningWalletAddress,
                banningWorkerName: options.banningWorkerName,
    ...
    (function init(){
        ...
        //Interval to look through bannedIPs, bannedWalletAddresses and bannedWorkerNames for old bans and remove them in order to prevent a memory leak
        if (options.banning && options.banning.enabled){
        ...
        if (options.banningWalletAddress && options.banningWalletAddress.enabled){
            setInterval(function(){
                for (walletAddress in bannedWalletAddresses){
                    var banTime = bannedWalletAddresses[walletAddress];
                    if (Date.now() - banTime > options.banningWalletAddress.time)
                        delete bannedWalletAddresses[walletAddress];
                }
            }, 1000 * options.banningWalletAddress.purgeInterval);
        }
        
        if (options.banningWorkerName && options.banningWorkerName.enabled){
            setInterval(function(){
                for (workerName in bannedWorkerNames){
                    var banTime = bannedWorkerNames[workerName];
                    if (Date.now() - banTime > options.banningWorkerName.time)
                        delete bannedWorkerNames[workerName];
                }
            }, 1000 * options.banningWorkerName.purgeInterval);
        }

        //SetupBroadcasting();
        if ((typeof(options.tlsOptions) !== 'undefined' 
            && typeof(options.tlsOptions.enabled) !== 'undefined') 
            && (options.tlsOptions.enabled === "true" 
            || options.tlsOptions.enabled === true)) {
            TLSoptions = {
    ...
    this.addBannedWalletAddress = function(walletAddress) {
        bannedWalletAddresses[walletAddress] = Date.now();
    }
    
    this.addBannedWorkerName = function(workerName) {
        bannedWorkerNames[workerName] = Date.now();
    }
